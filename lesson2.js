...
function power(val, pow) {
    if (pow > 1) {
        ...
        return 1 / (val * power(val, -pow - 1)); //для отрицательной степени
    }
}
...
0 - число.
все сравнения, кроме >= и <= выдают false, что понятно, потому что мы сравниваем 0 просто с отсутствующим значением
для лучшего понимания этого момента использовала статью https://habr.com/ru/company/ruvds/blog/337732/
по логике, null > 0 - это false, следовательно, обратная операция (<=) будет true. 
Этот момент сложно уложить в голове. 
Разве что при написании кода можно использовать, 
например, из базы товарных остатоков вытягивать строки не только где 0 стоит, но и где просто не заполнено(null)
*/